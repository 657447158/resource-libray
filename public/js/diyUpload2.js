    ...
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadStart', function (file) {
               webUploader.options.formData.guid=md5([file.id, file.name, file.size, file.type, file['__hash']].join(''));
            });
            ...
            webUploader.on('uploadSuccess', function (file, response) {
                if (webUploader.options.chunked) {
                    $.post('/merge_chunks', {
                        ...
                        $("#fileBox_"+data.fieldsId).attr("format",data.format);
                        $("#fileBox_"+data.fieldsId).attr("name",data.fieldsName);
                        $("#fileBox_"+data.fieldsId).attr("data-parentval",$("body").attr("type"));
                        if($(".bg_white").attr("type")=="picture" && data.format.toLowerCase()=="tif"){
                            $("#fileBox_"+data.fieldsId).append('<i class="iconfont chanSize">&#xC1117;</i>')
                            $("#fileBox_"+data.fieldsId).addClass("eps");
                        }
                        addFile(data.url,data.fieldsName,data.fieldsSize,data.format,data.fieldsId);
                    }, 'json');
                }
                var $fileBox = $('#fileBox_' + file.id);
                var $diyBar = $fileBox.find('.diyBar');
